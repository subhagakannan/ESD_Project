...
path = r'E:\ESDprojectGIT\ESDproject\sample images/test_video4.mp4'
...
while (cap.isOpened()):
    ...
    #        image_lines = frame
    if NumLines == 2:
        TwoLineCount = TwoLineCount+1
    elif NumLines ==1:
        OneLineCount = OneLineCount+1
    else:
        NoLinesCount = NoLinesCount+1
    print(NumLines);
    NumLines =0
    if(len(selectline) !=0 ):
    ...
    if ret == True:
        ...
        cv.rectangle(img=image_lines, pt1=(int(roi[0]), int(roi[1])), pt2=((int(roi[0])+int(roi[2])),(int(roi[1])+int(roi[3]))), color=(0, 0, 255), thickness=2)
...
cap.release()
cv.destroyAllWindows()
print("Information For tester")
print("Total Frames: "+str(TotalFrames))
print("Frames with 2 lane lines: "+str(TwoLineCount))
print("Frames with 1 lane line: "+str(OneLineCount))
print("Frames with No lane lines: "+str(NoLinesCount))

...
import Sign_Detection_Integrated_Video
import time
path = r'Img 2073-1.mp4'
...
while (cap.isOpened()):
    ...
    if(len(selectline) !=0 ):
        ...
        image_lines = frame
    OutputFrame = Sign_Detection_Integrated_Video.HSV_Proc(image_lines)
    FPS = round(1.0 / (time.time() - start_time))
    cv.putText(OutputFrame,("FPS:"+str(FPS)), (600, 35), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv.LINE_AA)
    if ret == True:
        firstFrame =0
        cv.namedWindow('frame2',cv.WND_PROP_FULLSCREEN)
        cv.setWindowProperty('frame2', cv.WND_PROP_FULLSCREEN, cv.WINDOW_FULLSCREEN)
        cv.imshow("frame2", OutputFrame)
        #print(distFrmCenter);
        print('-------------------------------------------------------------------');
